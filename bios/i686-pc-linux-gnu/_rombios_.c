...
static char bios_date_string[] = "$Date: 2003/09/25 03:12:52 $";

static char CVSID[] = "$Id: _rombios_.c,v 1.1.1.1 2003/09/25 03:12:52 fht Exp $";
# 1043 "rombios.c"
static struct {
  Bit16u normal;
  Bit16u shift;
...
print_bios_banner()
{
  ...
  bios_printf(2, "Bochs"" BIOS, %d cpu%s, ", 1, 1>1?"s":"");
  ...
  Bit8u cdboot; Bit16u drive;
{
  ...
  if(cdboot)i=2;
  else if((drive&0x0080)==0x00)i=0;
  else if((drive&0x0080)==0x80)i=1;
  else return;
  ...
  Bit8u cdboot; Bit8u drive;
{
  ...
  if (cdboot)
    bios_printf(4 | 2, "Boot from %s failed\n",drivetypes[2]);
  else if (drive & 0x80)
    bios_printf(4 | 2, "Boot from %s %d failed\n", drivetypes[1],drivenum);
  else
    bios_printf(4 | 2, "Boot from %s %d failed\n", drivetypes[0],drivenum);

  if (reason==0)
    bios_printf((2 | 4 | 1), "Not a bootable disk\n");
  else
    bios_printf((2 | 4 | 1), "Could not read the boot disk\n");
}
...
nmi_handler_msg()
{
  bios_printf(4, "NMI Handler called\n");
}
...
void ata_detect( )
{
  ...
  write_word(ebda_seg,&((ebda_data_t *) 0)->ata.channels[2].iobase2,0x3e8);
  ...
  write_word(ebda_seg,&((ebda_data_t *) 0)->ata.channels[3].iobase2,0x368);
    ...
    if(type == 0x03) {
      ...
      switch (type) {
        case 0x02:
          ...
          bios_printf(2, " ATA-%d Hard-Disk (%d MBytes)\n", version,(Bit16u)sizeinmb);
          break;
        case 0x03:
          bios_printf(2, "ata%d %s: ", channel,slave?" slave":"master");
          i=0; while(c=read_byte(get_SS(),model+i++)) bios_printf(2, "%c", c);
...
cdrom_boot()
{
  ...
  if(read_byte(ebda_seg,&((ebda_data_t *) 0)->cdemu.emulated_drive)!=0x00)
    bios_printf((2 | 4 | 1), "El-Torito: Cannot boot as a harddisk yet\n");
...
# 3564 "rombios.c"
      switch (regs.u.r8.al) {
        ...
        case 5:
          ...
          if (regs.u.r8.al == 5) {
            if (regs.u.r8.bh != 3)
              bios_printf((2 | 4 | 1), "INT 15h C2 AL=5, BH=%02x\n", (unsigned) regs.u.r8.bh);
            mouse_flags_2 = read_byte(ebda_seg, 0x0027);
        ...
        case 2:
          ...
          switch (regs.u.r8.bh) {
            ...
            case 6:
              ...
              break;
            default:
              bios_printf((2 | 4 | 1), "INT 15h C2 AL=2, BH=%02x\n", (unsigned) regs.u.r8.bh);
            }
          break;
  ...
  Bit16u DI, SI, BP, SP, BX, DX, CX, AX;
{
  ...
  switch (scancode) {
    ...
    default:
      ...
      if (scancode > 0x53) {
        bios_printf(4, "KBD: int09h_handler(): unknown scancode read!");
        return;
        }
      if (shift_flags & 0x08) {
...
# 4296 "rombios.c"
  ...
  if (buffer_tail == buffer_head) {
    bios_printf((2 | 4 | 1), "KBD: dropped key scan=%02x, ascii=%02x\n", (int) scan_code, (int) ascii_code);
  ...
  Bit16u make_farcall, Z, Y, X, status;
{
  ...
  if ( (mouse_flags_2 & 0x80) != 0x80 ) {
    bios_printf((2 | 4 | 1), "int74_function:\n");
    }
  ...
  Bit16u DI, SI, BP, SP, BX, DX, CX, AX, DS, ES, FLAGS;
{
  ...
  if ( (( DX & 0x00ff ) < 0x80) || (( DX & 0x00ff ) >= 0x80 + (4*2)) ) {
    bios_printf(4, "int13_harddisk: function %02x, DL out of range %02x\n", ( AX >> 8 ), ( DX & 0x00ff ));
    goto int13_fail;
    }


  device=read_byte(ebda_seg,&((ebda_data_t *) 0)->ata.hdidmap[( DX & 0x00ff )-0x80]);


  if (device >= (4*2)) {
    bios_printf(4, "int13_harddisk: function %02x, unmapped device for DL=%02x\n", ( AX >> 8 ), ( DX & 0x00ff ));
    goto int13_fail;
    }
  ...
  Bit16u DI, SI, BP, SP, BX, DX, CX, AX, DS, ES, FLAGS;
{
  ...
  if( (( DX & 0x00ff ) < 0xE0) || (( DX & 0x00ff ) >= 0xE0+(4*2)) ) {
    bios_printf(4, "int13_cdrom: function %02x, DL out of range %02x\n", ( AX >> 8 ), ( DX & 0x00ff ));
    goto int13_fail;
    }


  device=read_byte(ebda_seg,&((ebda_data_t *) 0)->ata.cdidmap[( DX & 0x00ff )-0xE0]);


  if (device >= (4*2)) {
    bios_printf(4, "int13_cdrom: function %02x, unmapped device for DL=%02x\n", ( AX >> 8 ), ( DX & 0x00ff ));
    goto int13_fail;
    }
  ...
  Bit16u DI, SI, BP, SP, BX, DX, CX, AX, ES, FLAGS;
{
  ...
  switch (( AX >> 8 )) {
    ...
    case 0x04:
      ...
      elba = (Bit16u)(vlba+nbsectors-1)/4;
...
# 7333 "rombios.c"
    ...
    case 0xb1:
      ...
      if (regs.u.r8.al == 0xff) {
        bios_printf(4, "PCI BIOS not present\n");
  ...
  iret_addr_t iret_addr;
{
  ...
  val8 = inb_cmos(0x0c);
  if (val8 == 0) bios_printf((2 | 4 | 1), "int70: regC 0\n");
  if (val8 & 0x40) bios_printf((2 | 4 | 1), "int70: periodic request\n");
  if (val8 & 0x20) {
