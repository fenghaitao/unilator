/////////////////////////////////////////////////////////////////////////
// $Id: dbg_main.cc,v 1.1.1.1 2003/09/25 03:12:53 fht Exp $
/////////////////////////////////////////////////////////////////////////
//
...
#ifdef BX_DEBUG_ARM
    bx_memsys.pluginMmu->dbg_xlate_linear2phy(sp, &paddr, &paddr_valid);
    if (paddr_valid) {
      if (BX_MEM(0)->dbg_fetch_mem(paddr, 2, buf))
        dbg_printf ( "   %08x [%08x]  %04x\n", sp, paddr, (int)buf[0] | ((int)buf[1] << 8));
      else
...
#ifdef BX_DEBUG_ARM
  bx_memsys.pluginMmu->dbg_xlate_linear2phy(BX_CPU(which_cpu)->guard_found.laddr,&phy,&valid);
#endif
  if (valid) {
    unsigned ilen;

#ifdef BX_DEBUG_ARM
    bx_memsys.read_virtual_dword(0,BX_CPU(which_cpu)->guard_found.laddr, &inst);
    ...
    if( BX_CPU(dbg_cpu)->trace_reg ) BX_CPU_THIS_PTR dbg_dump_cpu();
    BX_CPU_THIS_PTR dbg_print_insn(inst,BX_CPU(which_cpu)->guard_found.laddr);
  }
#endif
}
               ...
               Bit32u addr, bx_bool addr_passed, int simulator)
{
  ...
  for (i=1; i<=repeat_count; i++) {
    ...
    if (is_linear) {
#ifdef BX_DEBUG_ARM
      bx_memsys.pluginMmu->dbg_xlate_linear2phy(addr, &paddr, &paddr_valid);
#endif
      if (!paddr_valid) {
    ...
    bx_memsys.pluginMem->dbg_fetch_mem(paddr, data_size, databuf);
...
bx_dbg_setpmem_command(Bit32u addr, unsigned len, Bit32u val)
{
  ...
  is_OK = bx_memsys.pluginMem->dbg_set_mem(addr, len, buf);
  if (!is_OK) {
...
bx_dbg_disassemble_command(bx_num_range range)
{
  ...
  do {
#ifdef BX_DEBUG_ARM
    bx_memsys.pluginMmu->dbg_xlate_linear2phy(range.from, &paddr, &paddr_valid);
    if (paddr_valid) {
        bx_memsys.pluginMem->dbg_fetch_mem(paddr,4,(Bit8u *)&inst);
	dbg_printf("addr%8x\t%8x\t",range.from,inst);
        BX_CPU(0)->dbg_print_insn(inst,range.from);
    }
    else {
