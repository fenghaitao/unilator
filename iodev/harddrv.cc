/////////////////////////////////////////////////////////////////////////
// $Id: harddrv.cc,v 1.4 2003/11/12 08:41:06 fht Exp $
/////////////////////////////////////////////////////////////////////////
//
...
bx_hard_drive_c::init(void)
{
  ...
  BX_DEBUG(("Init $Id: harddrv.cc,v 1.4 2003/11/12 08:41:06 fht Exp $"));
  ...
  for (channel=0; channel<BX_MAX_ATA_CHANNEL; channel++) {
    if (bx_options.ata[channel].Opresent->get() == 1) {
	  ...
	  BX_HD_THIS channels[channel].irq));
       ...
       BX_HD_THIS raise_interrupt_timer = 0;
       BX_HD_THIS timer_handle = bx_pc_system.register_timer( this, timer_handler,
                                 10001, 1, 0,
...
bx_hard_drive_c::ready_to_send_atapi(Bit8u channel)
{
  ...
  void
bx_hard_drive_c::timer_handler(void *this_ptr)
{
    Bit8u channel = BX_HD_THIS current_channel ;

//    BX_INFO(("in bx_har_drvie_c::timer_handler"));  
    if (BX_HD_THIS raise_interrupt_timer){
          Bit32u irq = BX_HD_THIS channels[channel].irq; 
//          BX_INFO(("time out Raising interrupt %d {%s}", irq, BX_SELECTED_TYPE_STRING(channel)));
          DEV_pic_raise_irq(irq);
    	  BX_HD_THIS raise_interrupt_timer = 0;
    	  bx_pc_system.deactivate_timer(BX_HD_THIS timer_handle);
    }
}
...
bx_hard_drive_c::raise_interrupt(Bit8u channel)
{
    ...
    if (!BX_SELECTED_CONTROLLER(channel).control.disable_irq) {
    	  ...
    	  BX_HD_THIS raise_interrupt_timer = 1;
    	  BX_HD_THIS current_channel = channel;
	  bx_pc_system.activate_timer(BX_HD_THIS timer_handle, 
                         1001, 1);
      } else {
    	  BX_HD_THIS raise_interrupt_timer = 0;
          if (bx_dbg.disk || (BX_SELECTED_IS_CD(channel) && bx_dbg.cdrom))
