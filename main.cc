/////////////////////////////////////////////////////////////////////////
// $Id: main.cc,v 1.1.1.1 2003/09/25 03:12:51 fht Exp $
/////////////////////////////////////////////////////////////////////////
//
...
#if BX_SMP_PROCESSORS==1
  bx_memsys.init(bx_options.memory.Osize->get ()*1024*1024);	
//  BX_MEM(0)->init_memory(bx_options.memory.Osize->get () * 1024*1024);

  bx_memsys.reset(BX_RESET_HARDWARE);
...
//    BX_MEM(0)->load_ROM(bx_options.optrom[0].Opath->getptr (), bx_options.optrom[0].Oaddress->get ());
  bx_memsys.load_ROM(bx_options.optrom[0].Opath->getptr (), bx_options.optrom[0].Oaddress->get ());
...
//    BX_MEM(0)->load_ROM(bx_options.optrom[1].Opath->getptr (), bx_options.optrom[1].Oaddress->get ());
  bx_memsys.load_ROM(bx_options.optrom[1].Opath->getptr (), bx_options.optrom[1].Oaddress->get ());
...
//    BX_MEM(0)->load_ROM(bx_options.optrom[2].Opath->getptr (), bx_options.optrom[2].Oaddress->get ());
  bx_memsys.load_ROM(bx_options.optrom[2].Opath->getptr (), bx_options.optrom[2].Oaddress->get ());
...
//    BX_MEM(0)->load_ROM(bx_options.optrom[3].Opath->getptr (), bx_options.optrom[3].Oaddress->get ());
  bx_memsys.load_ROM(bx_options.optrom[3].Opath->getptr (), bx_options.optrom[3].Oaddress->get ());

  // Then Load the BIOS and VGABIOS
  //BX_MEM(0)->load_ROM(bx_options.rom.Opath->getptr (), bx_options.rom.Oaddress->get ());
  bx_memsys.load_ROM(bx_options.rom.Opath->getptr (), bx_options.rom.Oaddress->get ());
  //BX_MEM(0)->load_ROM(bx_options.vgarom.Opath->getptr (), 0xc0000);
  bx_memsys.load_ROM(bx_options.vgarom.Opath->getptr (), 0xc0000);

/* adding code to make cpu plugin 2004-02-24 */
  PLUG_load_plugin(cpu, PLUGTYPE_CORE);

  pluginCpu->init();
//  pluginCpu->init  (BX_MEM(0));
//  BX_CPU(0)->init (BX_MEM(0));
#if BX_SUPPORT_APIC
  BX_CPU(0)->local_apic.set_id (0);
  BX_INSTR_INIT(0);
#endif
  pluginCpu->reset(BX_RESET_HARDWARE);
