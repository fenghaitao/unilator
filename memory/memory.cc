/////////////////////////////////////////////////////////////////////////
// $Id: memory.cc,v 1.7 2003/11/08 07:24:23 fht Exp $
/////////////////////////////////////////////////////////////////////////
//
...
BX_MEM_C::writePhysicalPage(BX_CPU_C *cpu, Bit32u addr, unsigned len, void *data)
{
  ...
  if ((addr & 0xf0000000) == 0x80000000)  addr &= 0x0fffffff;
  else if ((addr & 0xff000000) == 0x7c000000)
  {
     //fprintf(stderr,"write io %8x \n",addr);	
     addr &= 0x00ffffff;
     
  }
  else if ((addr & 0xff000000) == 0x79000000){
	/* fixme too fht 2003-10-30 */
	ISASOURCE = *(Bit8u *)data;
	return;
  }
  else if ((addr & 0x42000000) == 0x42000000){
      //fprintf(stderr, "write address %8x,data is %8x\n",addr,*(Bit32u *)data);
      addr &= addr & 0x00ffffff;
      if (len !=4) BX_PANIC(("write invalid address %8x,length is not four \n",addr|0x42000000));  

      switch (addr)
      {
	case 0x108 : 
		IRQSOURCE |= *(Bit32u *)data;
		return;
	case 0x184 : 
		IRQSOURCE |= *(Bit32u *)data;
		return;
	case 0x284 : 
		FIRQSOURCE |= *(Bit32u *)data;
		return;
	case 0x188 : 
		IRQENABLE |= *(Bit32u *)data;
		return;
	case 0x288 : 
		FIRQENABLE |= *(Bit32u *)data;
		return;
	case 0x18c : 
		IRQENABLE &= ~(*(Bit32u *)data);
		return;
	case 0x28c : 
		FIRQENABLE &= ~(*(Bit32u *)data);
		return;
	default : BX_PANIC(("write invalid address %8x \n",addr|0x42000000));
      }
  }
  ...
  if (addr + len <= 0x0ffff){
     if (len == 4){ 
        BX_OUTP(addr,*(Bit32u *)data,4);
        return;
        }
     if (len == 2){ 
        BX_OUTP(addr,*(Bit16u *)data,2);
        return;
        }
     if (len == 1){
        BX_OUTP(addr,*(Bit8u *)data,1);
        return;
        }
  }


  if ( addr > BX_MEM_THIS len ) BX_PANIC(("write invalid address %8x \n",addr));
...
/* added for memory mapped io 2003-07-20 */
/*  if (a20addr + len <= 0x0ffff){ */
/*  simple mmu 2003-10-22*/
  if ( a20addr <= BX_MEM_THIS len ) {
...
BX_MEM_C::readPhysicalPage(BX_CPU_C *cpu, Bit32u addr, unsigned len, void *data)
{
  ...
  if ((addr & 0xf0000000) == 0x80000000)  addr &= 0x0fffffff;
  else if ((addr & 0x0ff000000) == 0x7c000000)  addr &= 0x00ffffff;
  else if ((addr & 0x0ff000000) == 0x79000000){
/* fixme fht 2003-10-30 */
	*(Bit8u *)data = ISASOURCE;
	return;
	}
  else if ((addr & 0x42000000) == 0x42000000){
      if (len !=4) BX_PANIC(("write invalid address %8x,length is not four \n",addr|0x42000000));  
      ...
      {
	case 0x108 : 
//		*(Bit32u *)data = IRQSOURCE&IRQENABLE ;
		break;
	case 0x180 : 
		*(Bit32u *)data = IRQSOURCE&IRQENABLE ;
		break;
	case 0x280 : 
//		*(Bit32u *)data = FIRQSOURCE&FIRQENABLE ;
		*(Bit32u *)data = 0 ;
		break;
	case 0x184 : 
		*(Bit32u *)data = IRQSOURCE ;
		break;
	case 0x284 : 
		*(Bit32u *)data = FIRQSOURCE ;
		break;
	case 0x188 : 
		*(Bit32u *)data = IRQENABLE ;
		break;
	case 0x288 : 
		*(Bit32u *)data = FIRQENABLE ;
		break;
	default : BX_PANIC(("write invalid address %8x \n",addr|0x42000000));
      }
      //fprintf(stderr, "read address %8x,data is %8x\n",addr,*(Bit32u *)data);
      return;
  }
  ...
  if (addr + len <= 0x0ffff){
     if (len == 4){ 
        *(Bit32u *)data=BX_INP(addr,4);
        return;
        }
     if (len == 2){ 
        *(Bit16u *)data=BX_INP(addr,2);
        return;
        }
     if (len == 1){
        *(Bit8u *)data=BX_INP(addr,1);
        return;
        }
  }

  if ( addr > BX_MEM_THIS len ) BX_PANIC(("read invalid address %8x \n",addr));
