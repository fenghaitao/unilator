...
class BOCHSAPI BX_MEM_C : public bx_mem_stub_c {

public:
/* debugging */
  unsigned int IRQENABLE;
  unsigned int FIRQENABLE;
  unsigned int IRQSOURCE;
  unsigned int FIRQSOURCE;
  unsigned int ISASOURCE;
  ...
  virtual void    readPhysicalPage(BX_CPU_C *cpu, Bit32u addr,
                                      unsigned len, void *data);
  virtual void    writePhysicalPage(BX_CPU_C *cpu, Bit32u addr,
                                       unsigned len, void *data);
  virtual void    load_ROM(const char *path, Bit32u romaddress);
  virtual Bit32u  get_memory_in_k(void);

#if (BX_DEBUGGER || BX_GDBSTUB)
  virtual bx_bool dbg_fetch_mem(Bit32u addr, unsigned len, Bit8u *buf);
  virtual bx_bool dbg_set_mem(Bit32u addr, unsigned len, Bit8u *buf);
  virtual bx_bool dbg_crc32(
    unsigned long (*f)(unsigned char *buf, int len),
    Bit32u addr1, Bit32u addr2, Bit32u *crc);
#endif

  virtual Bit8u * getHostMemAddr(BX_CPU_C *cpu, Bit32u a20Addr, unsigned op);  
};
